    ...
    def __init__(self):
        ...
        if not os.path.exists(os.path.join(os.path.dirname(__file__), "LocatorsObject.ini")):
            with open(os.path.join(os.path.dirname(__file__), "LocatorsObject.ini"), "w+") as f:
                ...
                conf.write(f)
        self.conf.read(filenames="LocatorsObject.ini")
...
class TestCase:
    def __init__(self,title,url,step,page,locator_name,index,data,data_two,data_three,data_transfer,exp,assertmethod):
        self.title = title
        self.url = url
...
class MainInit(QMainWindow):
    def __init__(self):
        ...
        self.default_teardown_value = "每个用例执行完关闭浏览器"
    ...
    def initUI(self):
        ...
        test_service_url_action = QAction("服务器IP及端口配置",self)
        ...
        self.url_line_edit.setPlaceholderText("请输入测试网址")
    ...
    def import_method(self):
        ...
        if path:
            ...
            sheet_1.cell(1, 2).value = "请求网址"
            sheet_1.cell(1, 3).value = "步骤"
            sheet_1.cell(1, 4).value = "预期结果"
            sheet_1.cell(1, 5).value = "断言方式"
            sheet_1.cell(1, 6).value = "文件路径"
            sum = 1
            for object_path,dirs,files in os.walk(path):
                ...
                for file in files:
                    if file.endswith(".web"):
                        ...
                        sheet_1.cell(sum, 2).value = test_case.url
                        step_list = []
                        ...
                        sheet_1.cell(sum, 3).value = str(step_list)
                        sheet_1.cell(sum, 4).value = test_case.exp
                        sheet_1.cell(sum, 5).value = test_case.assertmethod
                        sheet_1.cell(sum, 6).value = excute_path
        ...
        if path[0]:
            ...
            try:
                for i in range(0,max_row-1):
                    file_path = sheet_1.cell(index,6).value
                    base_key_list = []
                    page_list = []
                    locator_name_list = []
                    index_list = []
                    ...
                    data_transfer_list = []
                    try:
                        raw_data = eval(sheet_1.cell(index, 3).value)
                    except Exception:
                        QMessageBox.information(self, "提示", "第" + str(index) + "行第3列的数据必须是列表", QMessageBox.Yes)
                        return None
                    for j in range(0,len(raw_data)):
                        try:
                            ...
                            QMessageBox.information(self, "提示", "第" + str(index) + "行第3列中第"+str(j+1)+"个的数据必须是列表", QMessageBox.Yes)
                            return None
                    test_case = TestCase(sheet_1.cell(index,1).value, sheet_1.cell(index,2).value,base_key_list,
                                     page_list,locator_name_list,
                                     index_list,
                                     data_one_list, data_two_list,
                                     data_three_list,
                                     data_transfer_list, sheet_1.cell(index,4).value,
                                     sheet_1.cell(index,5).value)
                    pickle.dump(test_case, open(file_path, "wb"))
                    index = index + 1
            except Exception:
                QMessageBox.information(self, "提示","测试用例{}：条| 获取文件最大行为{}：行".format(len(raw_data),max_row), QMessageBox.Yes)
                return  None
    ...
    def locate_value_word_method(self):
        if os.path.exists(os.path.join(os.path.dirname(__file__), "LocatorsObject.ini")):
            ...
            with open(os.path.join(os.path.dirname(__file__), "LocatorsObject.ini"), "r+") as f:
                text = f.read()
            locator_edit.setText(text)
    ...
    def page_combox_list_method(self):
        try:
            ...
            if  self.page_combox_list[index].currentText() !="NONE":
                ...
                self.locator_name_combox_list[index].addItems(self.options_dict[self.page_combox_list[index].currentText()])
            else:
                self.locator_name_combox_list[index].clear()
                self.locator_name_combox_list[index].addItem(self.options_dict[self.page_combox_list[index].currentText()])
        except Exception:
    ...
    def resizeEvent(self,sizeEvent):
        ...
        self.url_line_edit.move(int(self.main_width/60+self.line_edit_width+10), int(self.main_height/16))
        ...
        self.chrome_radio.move(int(self.main_width/60+self.line_edit_width*2+20),int(self.main_height*2/16))
        self.ie_radio.move(int(self.main_width/60+self.line_edit_width*2+20+QApplication.desktop().width()/8-30),int(self.main_height*2/16))
        self.firefox_radio.move(int(self.main_width/60+self.line_edit_width*2+20+QApplication.desktop().width()/4-60),int(self.main_height*2/16))
    ...
    def save_test_case_action_method(self):
        ...
        if not self.url_line_edit.text():
            QMessageBox.information(self, "提示", "请输入测试网址", QMessageBox.Ok)
            return None
        ...
        if path[0]:
            for i in range(0,len(self.data_value_list)):
                ...
                test_case = TestCase(self.title_line_edit.text(),self.url_line_edit.text(),self.step_combox_text_list,
                                     self.page_combox_text_list,self.locator_name_combox_text_list,self.data_name_text_list,
                                     self.data_value_text_list,self.data_value_two_text_list,self.data_value_three_text_list,
                                     self.data_transfer_text_list,self.exp_line_edit.text(),self.assert_method_combox.currentText())
        ...
        for i in range(0, len(self.step_combox_list)):
                ...
                try:
                    ...
                    if len(eval(self.data_transfer_list[i].text()))==2:
                        if eval(self.data_transfer_list[i].text())[1] not in [1,2,3] :
                            QMessageBox.information(self, "提示", "第" + str(i + 1) + "步的数据传递列表中的第二个值必须为数字1,2,3中的一个！！！", QMessageBox.Ok)
                            return None
                    ...
                    QMessageBox.information(self, "提示", "第" + str(i + 1) + "步的数据传递格式有问题，数据必须是列表或元组类型！！！", QMessageBox.Ok)
                    return None
                if len(eval(self.data_transfer_list[i].text())) == 2 or len(eval(self.data_transfer_list[i].text())) == 3:#进入该条件，数据传递格式就是正确的，并给下一步设置值
            ...
            if self.page_combox_list[i].currentText()=="NONE":#判断是否存在元素定位参数
                ...
                if self.data_value_three_list[i].text():
                    sum = sum + 1
                para_num = len(inspect.getfullargspec(self.driver_test_para[self.step_combox_list[i].currentText()]).args)
                if sum != para_num:
                    del sum
                    del para_num
    ...
    def test_service_url_method(self):
        ...
        def submit_btn_method():
            with open(file_path, "w+") as f:
                f.write(self.ip_line_edit.text())
            dialog.close()
        submit_btn = QPushButton("确定",dialog)
        submit_btn.clicked.connect(submit_btn_method)
        remove_btn = QPushButton("取消",dialog)
        remove_btn.clicked.connect(remove_btn_method)
        submit_btn.resize(150,30)
        submit_btn.move(0,30)
        remove_btn.resize(150,30)
        remove_btn.move(150,30)
        self.ip_line_edit.resize(300,30)
        self.ip_line_edit.setPlaceholderText("请输入测试服务器地址及端口号")
        dialog.resize(300,60)
        with open(file_path, "r") as f:
            self.ip_line_edit.setText(f.read())
        dialog.exec_()

    def teardown_action_method(self):
        tear_down_list=["每个用例执行完关闭浏览器","无操作"]
